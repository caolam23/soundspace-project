/**
 * Lấy thống kê tỷ lệ nguồn nhạc (Upload, YouTube, Spotify, SoundCloud)
...
async function getMusicSourcesStats(range = '7d') {
  try {
    ...
    // Unwind playlist trước để có thể match từng track
    pipeline.push({ $unwind: '$playlist' });
    
    // Match theo thời gian nếu có startDate
    if (startDate) {
      pipeline.push({
        $match: { 'playlist.createdAt': { $gte: startDate } }
      });
    }
...
/**
 * Lấy top 5 người dùng đóng góp nhiều bài hát nhất
...
/**
 * Lấy thống kê số bài hát được thêm theo thời gian
 * @param {string} range - Khoảng thời gian: '1d', '7d', '1m', 'all'
...
async function getUserGrowth(range = '1m') {
  try {
    ...
    const pipeline = [
      // Lọc user trong khoảng thời gian
      {
        $match: {
    ...
    // Đầu tiên, lấy số user hiện có trước khoảng thời gian
    ...
    const results = [];
    const currentUserCount = await User.countDocuments().catch(() => 0);
...
/**
 * Lấy tổng số lượt truy cập (login) theo range
 * @param {string} range - '1d', '7d', '1m', 'all'
...
async function getTotalVisitsStats(range = 'all') {
  try {
    ...
    return await Visit.countDocuments(query);
  } catch (error) {
